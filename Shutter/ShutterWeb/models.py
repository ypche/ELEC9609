...
class Message(models.Model):
    ...
    def __str__(self):
        return ('from %s to %s at %s:%s %s/%s/%s'
                % (self.author, self.receiver, self.time.hour, self.time.minute, self.time.day, self.time.month, self.time.year))
...
class TopicComment(models.Model):
    topic = models.ForeignKey('Topic', related_name='TopicComment_Topic', on_delete=models.CASCADE, null=True, blank=True)
    comment = models.ForeignKey('self', related_name='TopicComment_Comment', on_delete=models.CASCADE, null=True, blank=True)
    content = models.CharField(max_length=500, null=True, blank=True)
    time = models.DateTimeField(default=timezone.now)
...
class NewsComment(models.Model):
    ...
    comment = models.ForeignKey('self', related_name='NewsComment_Comment', on_delete=models.CASCADE, null=True, blank=True)
    content = models.CharField(max_length=500, null=True, blank=True)
    time = models.DateTimeField(default=timezone.now)
...
class PhotoComment(models.Model):
    ...
    photo = models.ForeignKey('Photo', related_name='PhotoComment_Photo', on_delete=models.CASCADE, null=True, blank=True)
    comment = models.ForeignKey('self', related_name='PhotoComment_Comment', on_delete=models.CASCADE, null=True, blank=True)
    thumbs_up_number = models.IntegerField(null=True, blank=True)
